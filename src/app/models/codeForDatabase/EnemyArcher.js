...
this.onmessage = function (turnEvent) {
    ...
    let healthLow = healthLessThan50Percent(me);
    let closestOtherTeam = findClosestEnemy(me, grid);

    //if enemies are too close or health is low and still "deep" in the attack range
    if (closestOtherTeam.distance < 2 || (healthLow && closestOtherTeam.distance < me.attackRange)) {
        //decide to move away or attack (prevents infinite chasing) (is randomized 50/50)
        var result = Math.floor(Math.random());
        
        if(result == 0) {
            //move away from them
            this.postMessage({ result: findMoveDirection(getOppositeDirection(closestOtherTeam.direction), grid, me) });
        } else {
            this.postMessage({result: "Attack"});
        }
        
    } else if (closestOtherTeam.distance <= me.attackRange) {
        //attack if within range
        this.postMessage({result: "Attack"});
    } else {
        //no one is too close nor is anyone in range, 
